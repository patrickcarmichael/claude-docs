---
title: "Langgraph Complete Documentation"
description: "Complete formatted documentation for Langgraph"
...
- [How to interact with the deployment using RemoteGraph](#how-to-interact-with-the-deployment-using-remotegr)
- [Initializing the graph](#initializing-the-graph)
- [Invoking the graph](#invoking-the-graph)
- [invoke the graph](#invoke-the-graph)
- [stream outputs from the graph](#stream-outputs-from-the-graph)
- [invoke the graph](#invoke-the-graph)
- [stream outputs from the graph](#stream-outputs-from-the-graph)
- [Thread-level persistence](#thread-level-persistence)
- [create a thread (or use an existing thread instead)](#create-a-thread-or-use-an-existing-thread-instead)
- [invoke the graph with the thread config](#invoke-the-graph-with-the-thread-config)
- [verify that the state was persisted to the thread](#verify-that-the-state-was-persisted-to-the-thread)
- [Using as a subgraph](#using-as-a-subgraph)
- [define parent graph](#define-parent-graph)
- [add remote graph directly as a node](#add-remote-graph-directly-as-a-node)
- [invoke the parent graph](#invoke-the-parent-graph)
- [stream outputs from both the parent graph and subgraph](#stream-outputs-from-both-the-parent-graph-and-subg)
- [How to integrate LangGraph with AutoGen, CrewAI, and other frameworks](#how-to-integrate-langgraph-with-autogen-crewai-and)
- [Prerequisites](#prerequisites)
- [Setup](#setup)
- [1. Define AutoGen agent](#1-define-autogen-agent)
- [2. Create the graph](#2-create-the-graph)
- [Create the graph with memory for persistence](#create-the-graph-with-memory-for-persistence)
- [Build the graph](#build-the-graph)
- [Compile with checkpointer for persistence](#compile-with-checkpointer-for-persistence)
- [3. Test the graph locally](#3-test-the-graph-locally)
- [pass the thread ID to persist agent outputs for future interactions](#pass-the-thread-id-to-persist-agent-outputs-for-fu)
- [4. Prepare for deployment](#4-prepare-for-deployment)
- [5. Deploy to LangGraph Platform](#5-deploy-to-langgraph-platform)
- [Enable tracing for your application](#enable-tracing-for-your-application)
- [Learn more](#learn-more)
- [How to use the graph API](#how-to-use-the-graph-api)
- [Setup](#setup)
- [Define and update state](#define-and-update-state)
- [Define the schema for the input](#define-the-schema-for-the-input)
- [Define the schema for the output](#define-the-schema-for-the-output)
- [Define the overall schema, combining both input and output](#define-the-overall-schema-combining-both-input-and)
- [Define the node that processes the input and generates an answer](#define-the-node-that-processes-the-input-and-gener)
- [Build the graph with input and output schemas specified](#build-the-graph-with-input-and-output-schemas-spec)
- [Invoke the graph with an input and print the result](#invoke-the-graph-with-an-input-and-print-the-resul)
- [The overall state of the graph (this is the public state shared across nodes)](#the-overall-state-of-the-graph-this-is-the-public-)
- [Output from node_1 contains private data that is not part of the overall state](#output-from-node_1-contains-private-data-that-is-n)
- [The private data is only shared between node_1 and node_2](#the-private-data-is-only-shared-between-node_1-and)
- [Node 2 input only requests the private data available after node_1](#node-2-input-only-requests-the-private-data-availa)
- [Node 3 only has access to the overall state (no access to private data from node_1)](#node-3-only-has-access-to-the-overall-state-no-acc)
- [Connect nodes in a sequence](#connect-nodes-in-a-sequence)
- [node_3 does not see the private data.](#node_3-does-not-see-the-private-data)
- [Invoke the graph with the initial state](#invoke-the-graph-with-the-initial-state)
- [The overall state of the graph (this is the public state shared across nodes)](#the-overall-state-of-the-graph-this-is-the-public-)
- [Build the state graph](#build-the-state-graph)
- [Test the graph with a valid input](#test-the-graph-with-a-valid-input)
- [Add runtime configuration](#add-runtime-configuration)
- [1. Specify config schema](#1-specify-config-schema)
- [2. Define a graph that accesses the config in a node](#2-define-a-graph-that-accesses-the-config-in-a-nod)
- [3. Pass in configuration at runtime:](#3-pass-in-configuration-at-runtime)
...
- [Call the graph: here we call it to generate a list of jokes](#call-the-graph-here-we-call-it-to-generate-a-list-)
- [Create and control loops](#create-and-control-loops)
- [Define nodes](#define-nodes)
- [Define edges](#define-edges)
- [Async](#async)
- [Combine control flow and state updates with `Command`](#combine-control-flow-and-state-updates-with-comman)
- [Define graph state](#define-graph-state)
- [Define the nodes](#define-the-nodes)
- [NOTE: there are no edges between nodes A, B and C!](#note-there-are-no-edges-between-nodes-a-b-and-c)
- [Visualize your graph](#visualize-your-graph)
- [Stream outputs](#stream-outputs)
- [Supported stream modes](#supported-stream-modes)
- [Stream from an agent](#stream-from-an-agent)
- [Stream from a workflow](#stream-from-a-workflow)
- [Use the functional API](#use-the-functional-api)
- [Creating a simple workflow](#creating-a-simple-workflow)
- [Parallel execution](#parallel-execution)
- [Calling graphs](#calling-graphs)
- [Call other entrypoints](#call-other-entrypoints)
- [Streaming](#streaming)
- [Retry policy](#retry-policy)
- [This variable is just used for demonstration purposes to simulate a network failure.](#this-variable-is-just-used-for-demonstration-purpo)
- [Let's configure the RetryPolicy to retry on ValueError.](#lets-configure-the-retrypolicy-to-retry-on-valueer)
- [Caching Tasks](#caching-tasks)
- [Resuming after an error](#resuming-after-an-error)
- [This variable is just used for demonstration purposes to simulate a network failure.](#this-variable-is-just-used-for-demonstration-purpo)
- [Initialize an in-memory checkpointer for persistence](#initialize-an-in-memory-checkpointer-for-persisten)
- [Workflow execution configuration with a unique thread identifier](#workflow-execution-configuration-with-a-unique-thr)
- [This invocation will take ~1 second due to the slow_task execution](#this-invocation-will-take-1-second-due-to-the-slow)
...
- [Build multi-agent systems](#build-multi-agent-systems)
- [Handoffs](#handoffs)
- [Build a multi-agent system](#build-a-multi-agent-system)
- [Handoffs](#handoffs)
- [Define agents](#define-agents)
- [Define multi-agent graph](#define-multi-agent-graph)
- [Multi-turn conversation](#multi-turn-conversation)
- [Prebuilt implementations](#prebuilt-implementations)
- [How to pass custom run ID or set tags and metadata for graph runs in LangSmith](#how-to-pass-custom-run-id-or-set-tags-and-metadata)
- [TLDR](#tldr)
- [Generate a random UUID -- it must be a UUID](#generate-a-random-uuid-it-must-be-a-uuid)
- [Works with all standard Runnable methods](#works-with-all-standard-runnable-methods)
- [Setup](#setup)
- [Define the graph](#define-the-graph)
- [First we initialize the model we want to use.](#first-we-initialize-the-model-we-want-to-use)
- [For this tutorial we will use custom tool that returns pre-defined values for weather in two cities (NYC & SF)](#for-this-tutorial-we-will-use-custom-tool-that-ret)
- [Define the graph](#define-the-graph)
...
- [Invocation example with an agent](#invocation-example-with-an-agent)
- [Example agent setup](#example-agent-setup)
- [Invocation: reads the name from state (initially empty)](#invocation-reads-the-name-from-state-initially-emp)
- [Advanced tool features](#advanced-tool-features)
- [Default error handling (enabled by default)](#default-error-handling-enabled-by-default)
...
- [NOTE: we're passing the store object here when creating a workflow via entrypoint()](#note-were-passing-the-store-object-here-when-creat)
- [How to build a multi-agent network (functional API)](#how-to-build-a-multi-agent-network-functional-api)
- [Define a tool to signal intent to hand off to a different agent](#define-a-tool-to-signal-intent-to-hand-off-to-a-di)
- [define an agent](#define-an-agent)
- [define a task that calls an agent](#define-a-task-that-calls-an-agent)
- [define the multi-agent network workflow](#define-the-multi-agent-network-workflow)
- [Setup](#setup)
- [Travel agent example](#travel-agent-example)
- [Define travel advisor ReAct agent](#define-travel-advisor-react-agent)
- [Define hotel advisor ReAct agent](#define-hotel-advisor-react-agent)
- [How to add multi-turn conversation in a multi-agent application (functional API)](#how-to-add-multi-turn-conversation-in-a-multi-agen)
- [Define a tool to signal intent to hand off to a different agent](#define-a-tool-to-signal-intent-to-hand-off-to-a-di)
- [`workflow()` below handles the handoffs explicitly](#workflow-below-handles-the-handoffs-explicitly)
- [define an agent](#define-an-agent)
- [define a task that calls an agent](#define-a-task-that-calls-an-agent)
- [define the multi-agent network workflow](#define-the-multi-agent-network-workflow)
- [Setup](#setup)
- [%%capture --no-stderr](#capture-no-stderr)
- [Define travel advisor ReAct agent](#define-travel-advisor-react-agent)
- [Define hotel advisor ReAct agent](#define-hotel-advisor-react-agent)
- [Test multi-turn conversation](#test-multi-turn-conversation)
- [How to add thread-level persistence (functional API)](#how-to-add-thread-level-persistence-functional-api)
- [Setup](#setup)
- [Example: simple chatbot with short-term memory](#example-simple-chatbot-with-short-term-memory)
- [How to manage conversation history in a ReAct Agent](#how-to-manage-conversation-history-in-a-react-agen)
- [Setup](#setup)
- [Keep the original message history unmodified](#keep-the-original-message-history-unmodified)
- [This function will be added as a new node in ReAct agent graph](#this-function-will-be-added-as-a-new-node-in-react)
- [The messages returned by this function will be the input to the LLM.](#the-messages-returned-by-this-function-will-be-the)
- [Overwrite the original message history](#overwrite-the-original-message-history)
- [Summarizing message history](#summarizing-message-history)
- [How to integrate LangGraph (functional API) with AutoGen, CrewAI, and other frameworks](#how-to-integrate-langgraph-functional-api-with-aut)
- [Setup](#setup)
- [Define AutoGen agent](#define-autogen-agent)
- [Create the workflow](#create-the-workflow)
- [add short-term memory for storing conversation history](#add-short-term-memory-for-storing-conversation-his)
- [Run the graph](#run-the-graph)
- [pass the thread ID to persist agent outputs for future interactions](#pass-the-thread-id-to-persist-agent-outputs-for-fu)
- [filename: fibonacci_range.py](#filename-fibonacci_rangepy)
- [How to handle large numbers of tools](#how-to-handle-large-numbers-of-tools)
- [Setup](#setup)
- [Define the tools](#define-the-tools)
- [Abbreviated list of S&P 500 companies for demonstration](#abbreviated-list-of-sp-500-companies-for-demonstra)
- [Create a tool for each company and store it in a registry with a unique UUID as the key](#create-a-tool-for-each-company-and-store-it-in-a-r)
- [Define the graph](#define-the-graph)
- [Define the state structure using TypedDict.](#define-the-state-structure-using-typeddict)
- [and a list of selected tool IDs.](#and-a-list-of-selected-tool-ids)
- [Retrieve all available tools from the tool registry.](#retrieve-all-available-tools-from-the-tool-registr)
- [The agent function processes the current state](#the-agent-function-processes-the-current-state)
- [The select_tools function selects tools based on the user's last message content.](#the-select_tools-function-selects-tools-based-on-t)
- [Repeating tool selection](#repeating-tool-selection)
- [Next steps](#next-steps)
- [How to create a ReAct agent from scratch](#how-to-create-a-react-agent-from-scratch)
- [Setup](#setup)
- [Create ReAct agent](#create-react-agent)
- [Define our tool node](#define-our-tool-node)
- [Define the node that calls the model](#define-the-node-that-calls-the-model)
- [Define the conditional edge that determines whether to continue or not](#define-the-conditional-edge-that-determines-whethe)
...
- [We now add a conditional edge](#we-now-add-a-conditional-edge)
- [We now add a normal edge from `tools` to `agent`.](#we-now-add-a-normal-edge-from-tools-to-agent)
- [Now we can compile and visualize our graph](#now-we-can-compile-and-visualize-our-graph)
- [Use ReAct agent](#use-react-agent)
- [Helper function for formatting the stream nicely](#helper-function-for-formatting-the-stream-nicely)
- [How to force tool-calling agent to structure output](#how-to-force-tool-calling-agent-to-structure-outpu)
- [Setup](#setup)
- [Define model, tools, and graph state](#define-model-tools-and-graph-state)
- [Inherit 'messages' key from MessagesState, which is a list of chat messages](#inherit-messages-key-from-messagesstate-which-is-a)
- [Option 1: Bind output as tool](#option-1-bind-output-as-tool)
- [Force the model to use tools by passing tool_choice="any"](#force-the-model-to-use-tools-by-passing-tool_choic)
- [Define the function that calls the model](#define-the-function-that-calls-the-model)
- [Define the function that responds to the user](#define-the-function-that-responds-to-the-user)
- [Define the function that determines whether to continue or not](#define-the-function-that-determines-whether-to-con)
- [Define a new graph](#define-a-new-graph)
- [Define the two nodes we will cycle between](#define-the-two-nodes-we-will-cycle-between)
- [Set the entrypoint as `agent`](#set-the-entrypoint-as-agent)
- [We now add a conditional edge](#we-now-add-a-conditional-edge)
...
- [Set the entrypoint as `agent`](#set-the-entrypoint-as-agent)
- [We now add a conditional edge](#we-now-add-a-conditional-edge)
- [How to disable streaming for models that don't support it](#how-to-disable-streaming-for-models-that-dont-supp)
- [Without disabling streaming](#without-disabling-streaming)
- [Disabling streaming](#disabling-streaming)
- [How to create a ReAct agent from scratch (Functional API)](#how-to-create-a-react-agent-from-scratch-functiona)
- [Setup](#setup)
- [Create ReAct agent](#create-react-agent)
- [Usage](#usage)
...
- [Related](#related)
- [Template Applications](#template-applications)
- [Install the LangGraph CLI](#install-the-langgraph-cli)
- [Available Templates](#available-templates)
- [🌱 Create a LangGraph App](#create-a-langgraph-app)
- [Next Steps](#next-steps)
- [Tools](#tools)
- [Tool calling](#tool-calling)
- [-> AIMessage(tool_calls=[{'name': 'multiply', 'args': {'a': 2, 'b': 3}, ...}])](#aimessagetool_callsname-multiply-args-a-2-b-3)
- [Prebuilt tools](#prebuilt-tools)
...
- [Patterns](#patterns)
- [MCP endpoint in LangGraph Server](#mcp-endpoint-in-langgraph-server)
- [Requirements](#requirements)
- [Exposing an agent as MCP tool](#exposing-an-agent-as-mcp-tool)
- [Define input schema](#define-input-schema)
- [Define output schema](#define-output-schema)
- [Combine input and output](#combine-input-and-output)
- [Define the processing node](#define-the-processing-node)
- [Build the graph with explicit schemas](#build-the-graph-with-explicit-schemas)
- [Run the graph](#run-the-graph)
...
- [Self-Hosted Data Plane](#self-hosted-data-plane)
- [Requirements](#requirements)
- [Self-Hosted Data Plane](#self-hosted-data-plane)
- [Streaming](#streaming)
- [What’s possible with LangGraph streaming](#whats-possible-with-langgraph-streaming)
- [Functional API concepts](#functional-api-concepts)
- [Overview](#overview)
...
- [Parts of a deployment](#parts-of-a-deployment)
- [Learn more](#learn-more)
- [LangGraph Platform](#langgraph-platform)
- [Why use LangGraph Platform?](#why-use-langgraph-platform)
- [Tracing](#tracing)
- [Learn more](#learn-more)
- [Memory](#memory)
- [Short-term memory](#short-term-memory)
- [Long-term memory](#long-term-memory)
- [Node that *uses* the instructions](#node-that-uses-the-instructions)
- [Node that updates instructions](#node-that-updates-instructions)
- [InMemoryStore saves data to an in-memory dictionary. Use a DB-backed store in production use.](#inmemorystore-saves-data-to-an-in-memory-dictionar)
- [get the "memory" by ID](#get-the-memory-by-id)
- [search for "memories" within this namespace, filtering on content equivalence, sorted by vector similarity](#search-for-memories-within-this-namespace-filterin)
...
- [Generic / global handler catches calls that aren't handled by more specific handlers](#generic-global-handler-catches-calls-that-arent-ha)
- [Matches the "thread" resource and all actions - create, read, update, delete, search](#matches-the-thread-resource-and-all-actions-create)
- [over the generic handler for all actions on the "threads" resource](#over-the-generic-handler-for-all-actions-on-the-th)
- [Thread creation. This will match only on thread create actions](#thread-creation-this-will-match-only-on-thread-cre)
- [it will take precedence for any "create" actions on the "threads" resources](#it-will-take-precedence-for-any-create-actions-on-)
- [Reading a thread. Since this is also more specific than the generic @auth.on handler, and the @auth.on.threads handler,](#reading-a-thread-since-this-is-also-more-specific-)
- [Run creation, streaming, updates, etc.](#run-creation-streaming-updates-etc)
- [Assistant creation](#assistant-creation)
- [Common Access Patterns](#common-access-patterns)
- [In your auth handler:](#in-your-auth-handler)
- [Supported Resources](#supported-resources)
- [Next Steps](#next-steps)
- [FAQ](#faq)
- [Do I need to use LangChain to use LangGraph? What’s the difference?](#do-i-need-to-use-langchain-to-use-langgraph-whats-)
- [How is LangGraph different from other agent frameworks?](#how-is-langgraph-different-from-other-agent-framew)
- [Does LangGraph impact the performance of my app?](#does-langgraph-impact-the-performance-of-my-app)
- [Is LangGraph open source? Is it free?](#is-langgraph-open-source-is-it-free)
- [How are LangGraph and LangGraph Platform different?](#how-are-langgraph-and-langgraph-platform-different)
- [Is LangGraph Platform open source?](#is-langgraph-platform-open-source)
- [Does LangGraph work with LLMs that don't support tool calling?](#does-langgraph-work-with-llms-that-dont-support-to)
- [Does LangGraph work with OSS LLMs?](#does-langgraph-work-with-oss-llms)
- [Can I use LangGraph Studio without logging in to LangSmith](#can-i-use-langgraph-studio-without-logging-in-to-l)
- [What does "nodes executed" mean for LangGraph Platform usage?](#what-does-nodes-executed-mean-for-langgraph-platfo)
- [MCP](#mcp)
- [Multi-agent systems](#multi-agent-systems)
- [Multi-agent architectures](#multi-agent-architectures)
- [this is the agent function that will be called as tool](#this-is-the-agent-function-that-will-be-called-as-)
- [the simplest way to build a supervisor w/ tool-calling is to use prebuilt ReAct agent graph](#the-simplest-way-to-build-a-supervisor-w-tool-call)
- [define team 1 (same as the single supervisor example above)](#define-team-1-same-as-the-single-supervisor-exampl)
- [define team 2 (same as the single supervisor example above)](#define-team-2-same-as-the-single-supervisor-exampl)
- [define top-level supervisor](#define-top-level-supervisor)
- [define the flow explicitly](#define-the-flow-explicitly)
...
- [Queue scalability](#queue-scalability)
- [Resilience](#resilience)
- [Postgres resilience](#postgres-resilience)
- [Redis resilience](#redis-resilience)
- [Graph API concepts](#graph-api-concepts)
- [Graphs](#graphs)
- [State](#state)
- [{'graph_output': 'My name is Lance'}](#graph_output-my-name-is-lance)
- [this is supported](#this-is-supported)
- [and this is also supported](#and-this-is-also-supported)
- [Nodes](#nodes)
- [You can then create edges to/from this node by referencing it as `"my_node"`](#you-can-then-create-edges-tofrom-this-node-by-refe)
- [Edges](#edges)
- [`Send`](#send)
- [`Command`](#command)
...
- [LangGraph Control Plane](#langgraph-control-plane)
- [Control Plane UI](#control-plane-ui)
- [Control Plane API](#control-plane-api)
- [Control Plane Features](#control-plane-features)
...
- [LangGraph Data Plane](#langgraph-data-plane)
- [Server Infrastructure](#server-infrastructure)
- ["Listener" Application](#listener-application)
- [Postgres](#postgres)
- [Redis](#redis)
- [Data Plane Features](#data-plane-features)
- [Overview](#overview)
...
- [get the latest state snapshot](#get-the-latest-state-snapshot)
- [get a state snapshot for a specific checkpoint_id](#get-a-state-snapshot-for-a-specific-checkpoint_id)
- [Memory Store](#memory-store)
- [Find memories about food preferences](#find-memories-about-food-preferences)
- [Store with specific fields to embed](#store-with-specific-fields-to-embed)
- [Store without embedding (still retrievable, but not searchable)](#store-without-embedding-still-retrievable-but-not-)
- [We need this because we want to enable threads (conversations)](#we-need-this-because-we-want-to-enable-threads-con)
- [... Define the graph ...](#define-the-graph)
- [Compile the graph with the checkpointer and store](#compile-the-graph-with-the-checkpointer-and-store)
- [Invoke the graph](#invoke-the-graph)
- [First let's just say hi to the AI](#first-lets-just-say-hi-to-the-ai)
- [Invoke the graph](#invoke-the-graph)
- [Let's say hi again](#lets-say-hi-again)
...
- [Configuration File {#configuration-file-concepts}](#configuration-file-configuration-file-concepts)
