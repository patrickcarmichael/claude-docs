...
**Navigation:** [← Previous](./25-웹-개발.md) | [Index](./index.md) | [Next →](./27-primeiros-passos.md)
...
O Cursor vem com ferramentas que permitem ao agente ajudar a galera a escrever código. Isso inclui leitura de arquivos, edição, execução de comandos no terminal, busca na web por documentação e outras.

Ferramentas de leitura não exigem aprovação (ex.: ler arquivos, buscar no código). A galera pode usar [.cursorignore](/pt-BR/context/ignore-files) para impedir o agente de acessar arquivos específicos, mas, fora isso, leituras geralmente são permitidas sem aprovação. Para ações que podem exfiltrar dados sensíveis, exigimos aprovação explícita.

Modificar arquivos no workspace atual não exige aprovação explícita, com algumas exceções. Quando o agente altera arquivos, as mudanças são salvas imediatamente em disco. Recomendamos usar o Cursor em workspaces sob controle de versão, para que o conteúdo dos arquivos possa ser revertido a qualquer momento. Exigimos aprovação explícita antes de mudar arquivos que alterem a configuração do nosso IDE/CLI, como o arquivo de configurações do editor do workspace. No entanto, quem usa recarregamento automático ao mudar arquivos deve estar ciente de que alterações do agente podem disparar execução automática antes de ter a chance de revisar as mudanças.

Qualquer comando de terminal sugerido pelos agentes exige aprovação por padrão. Recomendamos revisar cada comando antes do agente executá-lo. Quem aceitar o risco pode optar por habilitar o agente para rodar todos os comandos sem aprovação. Incluímos um recurso de [allowlist](/pt-BR/agent/tools) no Cursor, mas não consideramos isso um controle de segurança. Algumas pessoas optam por permitir comandos específicos, mas é um sistema de melhor esforço e podem existir formas de contorno. Não recomendamos "Run Everything", que ignora quaisquer allowlists configuradas.

<div id="third-party-tool-calls">
  ## Chamadas de ferramentas de terceiros
</div>

O Cursor permite conectar ferramentas externas via [MCP](/pt-BR/context/mcp). Todas as conexões MCP de terceiros devem ser explicitamente aprovadas pelo usuário. Depois que um usuário aprova um MCP, por padrão cada chamada de ferramenta sugerida no Modo Agente para cada integração MCP externa deve ser explicitamente aprovada antes da execução.
...
<div id="workspace-trust">
  ## Confiança no workspace
</div>

O Cursor IDE oferece o recurso padrão de [confiança no workspace](https://code.visualstudio.com/docs/editing/workspaces/workspace-trust), que vem desativado por padrão. A confiança no workspace exibe um prompt ao abrir um novo workspace para escolher entre modo normal ou restrito. O modo restrito interrompe a IA e outros recursos pelos quais a galera normalmente usa o Cursor. A gente recomenda outras ferramentas, como um editor de texto básico, para trabalhar com repositórios nos quais você não confia.

A confiança no workspace pode ser ativada nas configurações do usuário seguindo estes passos:

1. Abra o arquivo user settings.json
...
Se você acha que encontrou uma vulnerabilidade no Cursor, segue o guia na nossa página de segurança no GitHub e envia o relatório por lá. Se não der pra usar o GitHub, também dá pra falar com a gente em [security@cursor.com](mailto:security@cursor.com).
...
# Billing
Source: https://docs.cursor.com/pt-BR/account/billing

Gerenciar assinaturas, reembolsos e faturas do Cursor 

<div id="how-do-i-access-billing-settings">
  ### How do I access billing settings?
</div>

Acessa o portal de cobrança pelo [Dashboard](https://cursor.com/dashboard) clicando em "Billing" no teu Dashboard. Isso abre um portal seguro para todas as tarefas de cobrança.

<div id="what-are-cursors-billing-cycles">
  ### What are Cursor's billing cycles?
</div>

Os ciclos de cobrança são mensais ou anuais, começando na data da tua assinatura. Contas de Teams são cobradas por assento, com cobrança proporcional para novos membros.

<div id="how-do-seats-work-for-teams-accounts">
  ### How do seats work for Teams accounts?
</div>

Contas de Teams cobram por assento (um por membro do Team). Ao adicionar membros no meio do ciclo, tu só és cobrado pelo tempo restante. Se um membro usou créditos e for removido, o assento permanece ocupado até o fim do ciclo — não há reembolsos proporcionais. Admins do Team podem gerenciar assentos pelo dashboard.
...
1. Vai para o [dashboard](https://cursor.com/dashboard) do Cursor
2. Clica em "Billing and Invoices" na barra lateral esquerda para ir à página de cobrança
3. Clica em "Manage subscription"
4. Clica em "Update subscription"
5. Seleciona "Yearly" ou "Monthly", depois clica em "Continue"

**Teams plan**

1. Vai para o [dashboard](https://cursor.com/dashboard) do Cursor
2. Clica em "Billing and Invoices" na barra lateral esquerda para ir à página de cobrança
3. Clica no botão "Upgrade Now" para mudar para cobrança anual

<Note>
  Tu só podes mudar de cobrança mensal para anual por autoatendimento. Para mudar de anual para mensal, fala com a gente em
...
Encontra todo o histórico de cobrança no portal de cobrança. Visualiza e baixa faturas atuais e passadas.

<div id="can-i-get-invoices-automatically-emailed-to-me">
  ### Can I get invoices automatically emailed to me?
</div>

As faturas precisam ser baixadas manualmente no portal de cobrança. A gente está desenvolvendo o envio automático por e-mail. Tu vais poder optar por receber assim que estiver disponível.

<div id="how-do-i-update-my-billing-information">
  ### How do I update my billing information?
</div>

Atualiza método de pagamento, nome da empresa, endereço e informações fiscais pelo portal de cobrança. A gente usa Stripe para transações seguras. As mudanças afetam apenas faturas futuras; não conseguimos modificar faturas anteriores.

<div id="how-do-i-cancel-my-subscription">
  ### How do I cancel my subscription?
</div>

Cancela tua assinatura na página Billing and Invoices clicando em "Manage Subscription" e depois em "Cancel subscription". O acesso continua até o fim do teu período de cobrança atual.

<div id="im-having-other-billing-issues-how-can-i-get-help">
  ### I'm having other billing issues. How can I get help?
</div>

Para dúvidas de cobrança não cobertas aqui, manda um e-mail para [hi@cursor.com](mailto:hi@cursor.com) a partir do e-mail vinculado à tua conta. Inclui os detalhes da conta e as tuas questões.



# Preços
Source: https://docs.cursor.com/pt-BR/account/pricing

Planos do Cursor e seus preços

Você pode experimentar o Cursor gratuitamente ou adquirir um plano individual ou de equipe.

<div id="individual">
  ## Individual
</div>

Todos os planos individuais incluem:

* Completações de tab ilimitadas
* Limites estendidos de uso do agent em todos os modelos
* Acesso ao Bugbot
* Acesso a Background Agents

Cada plano inclui uso cobrado pelos preços de inferência de modelo [API prices](/pt-BR/models#model-pricing):

* Pro inclui US\$ 20 de uso de agent via API + uso bônus adicional
* Pro Plus inclui US\$ 70 de uso de agent via API + uso bônus adicional
* Ultra inclui US\$ 400 de uso de agent via API + uso bônus adicional

A gente trabalha duro pra conceder capacidade bônus além do uso garantido incluído. Como modelos diferentes têm custos de API diferentes, tua escolha de modelo afeta a saída de tokens e a velocidade com que teu uso incluído é consumido. Tu pode ver o uso e o detalhamento de tokens no [teu dashboard](https://cursor.com/dashboard?tab=usage). Notificações de limite são mostradas com frequência no editor.

<img src="https://mintcdn.com/cursor/GnTEh_6SKR7li-hM/images/account/usage-limits.png?fit=max&auto=format&n=GnTEh_6SKR7li-hM&q=85&s=c42b8ad6347f6fe9d075647dfece934c" alt="Limites de uso" style={{ borderRadius: 16 }} data-og-width="666" width="666" data-og-height="394" height="394" data-path="images/account/usage-limits.png" data-optimize="true" data-opv="3" srcset="https://mintcdn.com/cursor/GnTEh_6SKR7li-hM/images/account/usage-limits.png?w=280&fit=max&auto=format&n=GnTEh_6SKR7li-hM&q=85&s=e1ca3b5ab64a6ba6f3ad7d75b1ee72e2 280w, https://mintcdn.com/cursor/GnTEh_6SKR7li-hM/images/account/usage-limits.png?w=560&fit=max&auto=format&n=GnTEh_6SKR7li-hM&q=85&s=d39dc254e7d059ac24310f26ea47dd5d 560w, https://mintcdn.com/cursor/GnTEh_6SKR7li-hM/images/account/usage-limits.png?w=840&fit=max&auto=format&n=GnTEh_6SKR7li-hM&q=85&s=153472f14b4c10ec9a096b7a9db59888 840w, https://mintcdn.com/cursor/GnTEh_6SKR7li-hM/images/account/usage-limits.png?w=1100&fit=max&auto=format&n=GnTEh_6SKR7li-hM&q=85&s=0dc8b0cdb5c0677e45c1a082d0002880 1100w, https://mintcdn.com/cursor/GnTEh_6SKR7li-hM/images/account/usage-limits.png?w=1650&fit=max&auto=format&n=GnTEh_6SKR7li-hM&q=85&s=cf4ac9875f5e587d16d4c6f1e7fdc057 1650w, https://mintcdn.com/cursor/GnTEh_6SKR7li-hM/images/account/usage-limits.png?w=2500&fit=max&auto=format&n=GnTEh_6SKR7li-hM&q=85&s=13f21676cda8e4c2973bc3371892f4aa 2500w" />

<div id="how-much-usage-do-i-need">
  ### De quanta utilização eu preciso?
</div>

Com base nos nossos dados de uso, você pode esperar os seguintes níveis de uso:

* **Usuários diários do Tab**: Sempre ficam dentro de \$20
* **Usuários com uso limitado do Agent**: Geralmente ficam dentro dos \$20 incluídos
* **Usuários diários do Agent**: Normalmente $60–$100/mês de uso total
* **Power users (múltiplos agents/automação)**: Frequentemente \$200+/mês de uso total

Com base nos nossos dados de uso, os limites são aproximadamente equivalentes ao seguinte para um *usuário mediano*:

* Pro: \~225 requisições ao Sonnet 4, \~550 requisições ao Gemini ou \~500 requisições ao GPT 5
* Pro+: \~675 requisições ao Sonnet 4, \~1.650 requisições ao Gemini ou \~1.500 requisições ao GPT 5
* Ultra: \~4.500 requisições ao Sonnet 4, \~11.000 requisições ao Gemini ou \~10.000 requisições ao GPT 5

<div id="what-happens-when-i-reach-my-limit">
  ### O que acontece quando eu chego no meu limite?
</div>

Quando tu ultrapassar teu uso mensal incluso, vai aparecer um aviso no editor e tu pode escolher:

* **Adicionar uso sob demanda**: Continuar usando o Cursor nas mesmas tarifas de API com cobrança conforme o uso (pay‑as‑you‑go)
* **Fazer upgrade do teu plano**: Ir para um nível mais alto pra ter mais uso incluso

O uso sob demanda é cobrado mensalmente nas mesmas tarifas do teu uso incluso. As solicitações nunca são rebaixadas em qualidade ou velocidade.

<div id="teams">
  ## Times
</div>

Existem dois planos para times: Teams (US\$40/usuário/mês) e Enterprise (Personalizado).

Os planos para times oferecem recursos adicionais como:

* Aplicação do Privacy Mode
* Admin Dashboard com estatísticas de uso
* Faturamento centralizado do time
* SSO via SAML/OIDC

A gente recomenda o Teams pra qualquer cliente que consegue se autogerenciar. A gente recomenda o [Enterprise](/pt-BR/contact-sales) pra clientes que precisam de suporte prioritário, uso compartilhado, faturamento por invoice, SCIM ou controles avançados de segurança.

Saiba mais sobre os [preços do Teams](/pt-BR/account/teams/pricing).

<div id="auto">
  ## Auto
</div>

Ativar o Auto permite que o Cursor selecione o modelo premium mais adequado para a tarefa em questão, com a maior confiabilidade conforme a demanda atual. Esse recurso detecta queda de desempenho nas respostas e troca de modelo automaticamente para resolver o problema.
...
<Note>Investimos bastante na qualidade e no desempenho geral do Auto. A partir da sua próxima renovação de cobrança após 15 de setembro, o Auto consome uso nas seguintes tarifas de API.</Note>

* **Input + Cache Write**: US\$ 1,25 por 1M tokens
* **Output**: US\$ 6,00 por 1M tokens
* **Cache Read**: US\$ 0,25 por 1M tokens

Tanto o editor quanto o dashboard vão mostrar teu uso, incluindo o Auto. Se preferir selecionar um modelo diretamente, o uso é cobrado pelo preço de tabela da API desse modelo.

<div id="max-mode">
  ## Modo Max
</div>

Alguns modelos podem usar o [Modo Max](/pt-BR/models#max-mode), que permite raciocínios mais longos e janelas de contexto maiores, de até 1M de tokens. Embora a maioria das tarefas de codificação não precise do Modo Max, ele pode ser útil para consultas mais complexas, especialmente com arquivos grandes ou bases de código extensas. Usar o Modo Max consome mais recursos. Tu podes ver todas as solicitações e a divisão de tokens no [teu dashboard](https://cursor.com/dashboard?tab=usage).

<div id="bugbot">
  ## Bugbot
</div>

O Bugbot é um produto separado das assinaturas do Cursor e tem seu próprio plano de preços.

* **Pro** (US\$ 40/mês): Revisões ilimitadas em até 200 PRs por mês, acesso ilimitado ao Cursor Ask, integração com o Cursor para corrigir bugs e acesso às Regras do Bugbot
* **Teams** (US\$ 40/usuário/mês): Revisões de código ilimitadas em todos os PRs, acesso ilimitado ao Cursor Ask, uso compartilhado pela tua equipe e regras e configurações avançadas
* **Enterprise** (Personalizado): Tudo do Teams, mais análises e relatórios avançados, suporte prioritário e gerenciamento de conta

Saiba mais sobre os [preços do Bugbot](https://cursor.com/bugbot#pricing).

<div id="background-agent">
  ## Agente em Segundo Plano
</div>

Agentes em segundo plano são cobrados conforme a precificação da API do [modelo](/pt-BR/models) selecionado. Quando começar a usar, você vai definir um limite de gasto pros Agentes em Segundo Plano.

<Info>
  O uso de computação em Máquina Virtual (VM) para agentes em segundo plano será precificado no futuro.
</Info>



# Admin API
Source: https://docs.cursor.com/pt-BR/account/teams/admin-api
...
1. Navega até **cursor.com/dashboard** → guia **Settings** → **Cursor Admin API Keys**
...
# API de Rastreamento de Código com IA
Source: https://docs.cursor.com/pt-BR/account/teams/ai-code-tracking-api

Acesse análises de código geradas por IA nos repositórios da sua equipe
...
Para instruções detalhadas de autenticação, confere [Admin API authentication](/pt-BR/account/teams/admin-api#authentication).
...
<div className="full-width-table">
  ...
  | `user`      | string | Não         | Filtro opcional por um único usuário. Aceita email (por exemplo, [developer@company.com](mailto:developer@company.com)), ID codificado (por exemplo, user\_abc123...) ou ID numérico (por exemplo, 42) |                                                                                                                     |
</div>

<Note>
  As respostas retornam userId como um ID externo codificado com o prefixo user\_. Esse valor é estável para consumo via API.
</Note>
...
<div className="full-width-table">
  ...
  | `userId`               | string  | ID de usuário codificado (ex.: user\_abc123) |                                       |
...
<div className="full-width-table">
  ...
  | `userId`            | string | ID de usuário codificado (por exemplo, user\_abc123)                    |                                   |
  | `userEmail`         | string | Endereço de e-mail do usuário                                           |                                   |
...
* metadata\_json é um array de metadados serializado em JSON (pode omitir fileName no modo de privacidade)
...
# Analytics
Source: https://docs.cursor.com/pt-BR/account/teams/analytics

Acompanhe o uso da equipe e métricas de atividade

Admins da equipe podem acompanhar métricas no [dashboard](/pt-BR/account/teams/dashboard).
...
Veja métricas agregadas da equipe, incluindo total de abas e requisições premium. Para equipes com menos de 30 dias, as métricas refletem o uso desde a criação, incluindo a atividade anterior à entrada dos membros.

<div id="per-active-user">
  ### Per Active User
</div>

Veja as métricas médias por usuário ativo: abas aceitas, linhas de código e requisições premium.

<div id="user-activity">
  ### User Activity
</div>

Acompanhe usuários ativos semanais e mensais.

<div id="analytics-report-headers">
  ## Cabeçalhos do Relatório de Analytics
</div>

Quando exporta dados de analytics do dashboard, o relatório inclui métricas detalhadas sobre comportamento de usuários e uso de recursos. Aqui está o que cada cabeçalho significa:
...
<ResponseField name="Date" type="ISO 8601 timestamp">
  A data em que os dados de analytics foram registrados (ex.: 2024-01-15T04:30:00.000Z)
</ResponseField>

<ResponseField name="User ID" type="string">
  Identificador único de cada usuário no sistema
</ResponseField>

<ResponseField name="Email" type="string">
  Endereço de e-mail do usuário associado à conta
</ResponseField>

<ResponseField name="Is Active" type="boolean">
  Indica se o usuário estava ativo nessa data
</ResponseField>

<div id="ai-generated-code-metrics">
  ### Métricas de Código Gerado por IA
</div>

<ResponseField name="Chat Suggested Lines Added" type="number">
  Total de linhas de código sugeridas pelo recurso de chat com IA
</ResponseField>

<ResponseField name="Chat Suggested Lines Deleted" type="number">
  Total de linhas de código sugeridas para exclusão pelo chat com IA
</ResponseField>

<ResponseField name="Chat Accepted Lines Added" type="number">
  Linhas sugeridas pela IA que o usuário aceitou e adicionou ao código
</ResponseField>

<ResponseField name="Chat Accepted Lines Deleted" type="number">
  Exclusões sugeridas pela IA que o usuário aceitou
</ResponseField>
...
<ResponseField name="Chat Total Applies" type="number">
  Quantas vezes um usuário aplicou mudanças geradas pela IA a partir do chat
</ResponseField>

<ResponseField name="Chat Total Accepts" type="number">
  Quantas vezes um usuário aceitou sugestões da IA
</ResponseField>

<ResponseField name="Chat Total Rejects" type="number">
  Quantas vezes um usuário rejeitou sugestões da IA
</ResponseField>

<ResponseField name="Chat Tabs Shown" type="number">
  Quantas vezes abas de sugestões da IA foram exibidas para o usuário
</ResponseField>

<ResponseField name="Tabs Accepted" type="number">
  Abas de sugestões da IA que foram aceitas pelo usuário
</ResponseField>

<div id="request-type-metrics">
  ### Métricas por Tipo de Solicitação
</div>

<ResponseField name="Edit Requests" type="number">
  Solicitações feitas pelo recurso de composer/edit (edições inline com Cmd+K)
</ResponseField>

<ResponseField name="Ask Requests" type="number">
  Solicitações de chat em que usuários fizeram perguntas para a IA
</ResponseField>

<ResponseField name="Agent Requests" type="number">
  Solicitações feitas a agentes de IA (assistentes de IA especializados)
</ResponseField>

<ResponseField name="Cmd+K Usages" type="number">
  Quantas vezes o command palette (Cmd+K ou Ctrl+K) foi usado
</ResponseField>

<div id="subscription-and-api-metrics">
  ### Métricas de Assinatura e API
</div>

<ResponseField name="Subscription Included Reqs" type="number">
  Solicitações de IA cobertas pelo plano de assinatura do usuário
</ResponseField>

<ResponseField name="API Key Reqs" type="number">
  Solicitações feitas usando chaves de API para acesso programático
</ResponseField>

<ResponseField name="Usage-Based Reqs" type="number">
  Solicitações que contam para cobrança baseada em uso
</ResponseField>
...
<ResponseField name="Bugbot Usages" type="number">
  Quantas vezes o recurso de IA para detecção/correção de bugs foi usado
</ResponseField>
...
<ResponseField name="Most Used Model" type="string">
  O modelo de IA que o usuário usou com mais frequência (ex.: GPT-4, Claude)
</ResponseField>

<ResponseField name="Most Used Apply Extension" type="string">
  Extensão de arquivo mais usada ao aplicar sugestões de IA (ex.: .ts, .py, .java)
</ResponseField>

<ResponseField name="Most Used Tab Extension" type="string">
  Extensão de arquivo mais usada com recursos de conclusão por tab
</ResponseField>

<ResponseField name="Client Version" type="string">
  Versão do editor Cursor em uso
</ResponseField>
...
O relatório também inclui dados processados que ajudam a entender a contribuição de código da IA:

* Total de Linhas Adicionadas/Excluídas: contagem bruta de todas as mudanças de código
* Linhas Aceitas Adicionadas/Excluídas: linhas que se originaram de sugestões da IA e foram aceitas
* Solicitações do Composer: solicitações feitas pelo composer inline
* Solicitações de Chat: solicitações feitas pela interface de chat

<Note>
  Todos os valores numéricos têm padrão 0 se não estiverem presentes, valores booleanos têm padrão false e valores string têm padrão de strings vazias. As métricas são agregadas diariamente por usuário.
</Note>



# Analytics V2
Source: https://docs.cursor.com/pt-BR/account/teams/analyticsV2

Rastreamento avançado de métricas de uso e atividade da equipe

A gente tá trabalhando no lançamento da V2 da nossa infraestrutura de analytics. Isso inclui um refactor na forma como a gente rastreia várias métricas.

A partir de **1º de setembro de 2025**, e para quem estiver na **versão 1.5 do Cursor**, o analytics vai usar nossa infraestrutura V2. Versões anteriores subcontabilizavam várias métricas, incluindo:

* Total de linhas de código aceitas
* Total de linhas de código sugeridas
* Total de abas aceitas

Fica de olho enquanto a gente continua investindo em analytics e lançando novos recursos nessa área.



# Dashboard
Source: https://docs.cursor.com/pt-BR/account/teams/dashboard

Gerencia faturamento, uso e configurações da equipe pelo teu dashboard

O dashboard permite acessar o faturamento, configurar preços baseados em uso e gerenciar tua equipe.

<div id="overview">
  ## Visão geral
</div>

Vê um resumo rápido da atividade da tua equipe, estatísticas de uso e mudanças recentes. A página de visão geral oferece uma visão instantânea do teu workspace.
...
Define preferências e ajustes de segurança para toda a equipe. A página de configurações inclui:

<div id="teams-enterprise-settings">
  ## Configurações de Times e Enterprise
</div>

<AccordionGroup>
  <Accordion title="Privacy Settings">
    Controla as preferências de compartilhamento de dados do teu time. Configura políticas de retenção zero de dados com provedores de IA (OpenAI, Anthropic, Google Vertex AI, xAi Grok) e gerencia a aplicação de privacidade em todo o time.
  </Accordion>

  {" "}

  <Accordion title="Usage-Based Pricing Settings">
    Ativa a precificação baseada em uso e define limites de gastos. Configura limites mensais de gastos do time e limites opcionais por usuário. Controla se só admins podem
  ...
  <Accordion title="Bedrock IAM Role">
    Configura roles do IAM do AWS Bedrock para integração segura na nuvem.
  </Accordion>

  {" "}

  <Accordion title="Single Sign-On (SSO)">
    Configura autenticação SSO para times Enterprise pra simplificar o acesso dos usuários e
  ...
  <Accordion title="Cursor Admin API Keys">
    Cria e gerencia chaves de API para acesso programático aos recursos de administração do Cursor.
  </Accordion>

  {" "}

  <Accordion title="Active Sessions">
    Monitora e gerencia sessões de usuário ativas em todo o teu time.
  </Accordion>

  <Accordion title="Invite Code Management">
    Cria e gerencia códigos de convite pra adicionar novos membros ao time.
  </Accordion>

  <Accordion title="API Endpoints">
    Acessa os endpoints da API REST do Cursor para integração programática. Todos os endpoints da API estão disponíveis nos planos Team e Enterprise, exceto a [AI Code Tracking API](/pt-BR/docs/account/teams/ai-code-tracking-api), que requer o plano Enterprise.
  </Accordion>
</AccordionGroup>

<div id="enterprise-only-settings">
  ## Configurações exclusivas do Enterprise
</div>

<AccordionGroup>
  {" "}

  <Accordion title="Controle de acesso a modelos">
    Controla quais modelos de IA ficam disponíveis para o teu time. Define restrições em
    modelos específicos ou em camadas de modelos para gerenciar custos e garantir o uso adequado
    em toda a organização.
  </Accordion>

  {" "}

  <Accordion title="Configuração de execução automática (0.49+)">
    Configura as opções de execução automática de comandos para o Cursor versão 0.49 e
    posteriores. Controla quais comandos podem ser executados automaticamente e define políticas
    de segurança para execução de código.
  </Accordion>

  <Accordion title="Lista de bloqueio de repositórios">
    Impede o acesso a repositórios específicos por motivos de segurança ou conformidade.
  </Accordion>

  {" "}

  <Accordion title="Configuração do MCP (0.51+)">
    Configura o Model Context Protocol para o Cursor versão 0.51 e posteriores.
    Gerencia como os modelos acessam e processam o contexto do teu ambiente de
    desenvolvimento.
  </Accordion>

  {" "}

  <Accordion title="Configuração do Cursor Ignore (0.50+)">
    Define padrões de ignore para arquivos e diretórios no Cursor versão 0.50 e
    posteriores. Controla quais arquivos e diretórios são excluídos da análise e das
    sugestões da IA.
  </Accordion>

  <Accordion title="Proteção do diretório .cursor (0.51+)">
    Protege o diretório .cursor contra acesso não autorizado na versão 0.51 e posteriores. Garante que arquivos sensíveis de configuração e cache permaneçam protegidos.
  </Accordion>

  <Accordion title="API de rastreamento de código por IA">
    Acessa análises detalhadas de código gerado por IA nos repositórios do teu time. Recupera métricas de uso de IA por commit e alterações aceitas geradas por IA de forma granular por meio de endpoints REST. Requer o plano Enterprise. Encontra mais informações [aqui](/pt-BR/account/teams/ai-code-tracking-api).
  </Accordion>
</AccordionGroup>

<Note>
  O provisionamento **SCIM** (System for Cross-domain Identity Management) também está
  disponível para planos Enterprise. Consulta nossa [documentação do SCIM](/pt-BR/account/teams/scim)
...
Gerencia os membros da tua equipe, convida novos usuários e controla as permissões de acesso. Define permissões baseadas em função e acompanha a atividade dos membros.
...
Conecta o Cursor às tuas ferramentas e serviços favoritos. Configura integrações com sistemas de controle de versão, ferramentas de gerenciamento de projetos e outros serviços para desenvolvedores.
...
Para equipes enterprise, gerencia a autenticação de usuários e o acesso por meio da integração com o Active Directory. Configura SSO e o provisionamento de usuários.

<div id="usage">
  ## Uso
</div>

Acompanha métricas detalhadas de uso, incluindo requisições de IA, utilização de modelos e consumo de recursos. Monitora o uso entre membros da equipe e projetos.
...
Gerencia tua assinatura, atualiza os métodos de pagamento e acessa o histórico de cobrança. Baixa as faturas e gerencia as configurações de preços por uso.
...
Source: https://docs.cursor.com/pt-BR/account/teams/enterprise-settings

Gerencia centralmente as configurações do Cursor para tua organização

<div id="enterprise-settings">
  # Configurações empresariais
</div>

Dá pra gerenciar centralmente recursos específicos do Cursor por meio de soluções de gerenciamento de dispositivos, garantindo que ele atenda às necessidades da tua organização. Quando tu especifica uma política do Cursor, o valor dela substitui a configuração correspondente do Cursor nos dispositivos dos usuários.

Editor de configurações mostrando que a configuração “Extensions: Allowed” é gerenciada pela organização.

Atualmente, o Cursor oferece políticas para controlar os seguintes recursos administrados:

| Policy            | Description                                                                                                           | Cursor setting           | Available since |
| ----------------- | --------------------------------------------------------------------------------------------------------------------- | ------------------------ | --------------- |
| AllowedExtensions | Controla quais extensões podem ser instaladas.                                                                        | extensions.allowed       | 1.2             |
| AllowedTeamId     | Controla quais IDs de equipe podem fazer login. Usuários com IDs de equipe não autorizados são desconectados à força. | cursorAuth.allowedTeamId | 1.3             |

<div id="configure-allowed-extensions">
  ## Configurar extensões permitidas
</div>

A configuração `extensions.allowed` do Cursor controla quais extensões podem ser instaladas. Essa configuração aceita um objeto JSON em que as chaves são nomes de publisher e os valores são booleanos indicando se as extensões desse publisher são permitidas.

Por exemplo, definir `extensions.allowed` como `{"anysphere": true, "github": true}` permite extensões dos publishers Anysphere e GitHub, enquanto definir como `{"anysphere": false}` bloqueia extensões da Anysphere.

Pra gerenciar centralmente as extensões permitidas na tua organização, configura a política `AllowedExtensions` usando tua solução de gerenciamento de dispositivos. Essa política substitui a configuração `extensions.allowed` nos dispositivos dos usuários. O valor dessa política é uma string JSON que define os publishers permitidos.

Se quiser saber mais sobre extensões no Cursor, dá uma olhada na documentação de extensões.

<div id="configure-allowed-team-ids">
  ## Configurar IDs de equipe permitidos
</div>

A configuração do Cursor `cursorAuth.allowedTeamId` controla quais IDs de equipe podem fazer login no Cursor. Essa configuração aceita uma lista, separada por vírgulas, de IDs de equipe autorizados para acesso.
...
Quando alguém tenta fazer login com um ID de equipe que não está na lista permitida:
...
O Cursor oferece suporte à Diretiva de Grupo baseada no Registro do Windows. Quando as definições de diretiva estão instaladas, admins podem usar o Local Group Policy Editor para gerenciar os valores das diretivas.

Para adicionar uma diretiva:
...
<Accordion title="Arquivo .mobileconfig de exemplo">
...
**Importante:** O arquivo `.mobileconfig` fornecido inicializa **todas** as políticas disponíveis naquela versão do Cursor. Exclui as políticas que não forem necessárias.

Se tu não editar ou remover uma política do `.mobileconfig` de exemplo, essa política vai ser aplicada com o valor padrão (restritivo).

Instala manualmente um perfil de configuração dando um duplo clique no arquivo `.mobileconfig` no Finder e depois habilitando em Preferências do Sistema em **Geral** > **Gerenciamento de Dispositivos**. Remover o perfil em Preferências do Sistema vai remover as políticas do Cursor.

Pra mais informações sobre perfis de configuração, confere a documentação da Apple.
...
# Membros e papéis
Source: https://docs.cursor.com/pt-BR/account/teams/members

Gerenciar membros e papéis da equipe

As equipes do Cursor têm três papéis:

<div id="roles">
  ## Papéis
</div>

**Membros** são o papel padrão com acesso aos recursos Pro do Cursor.

* Acesso completo aos recursos Pro do Cursor
* Sem acesso às configurações de cobrança nem ao painel de administração
* Podem ver o próprio uso e o orçamento restante baseado em uso

**Admins** controlam o gerenciamento da equipe e as configurações de segurança.

* Acesso completo aos recursos Pro
* Adicionar/remover membros, modificar papéis, configurar SSO
* Configurar preços baseados em uso e limites de gastos
* Acesso às análises da equipe

**Admins não pagos** gerenciam equipes sem ocupar uma licença paga — ideal para pessoal de TI ou finanças que não precisa de acesso ao Cursor.

* Não faturável, sem recursos Pro
* Mesmas capacidades administrativas que Admins

<Info>Admins não pagos exigem pelo menos um usuário pago na equipe.</Info>
...
<div className="full-width-table">
  | Capacidades              | Membro | Admin | Admin sem assento pago |
  | ------------------------ | :----: | :---: | :--------------------: |
  | Usar recursos do Cursor  |    ✓   |   ✓   |                        |
  | Convidar membros         |    ✓   |   ✓   |            ✓           |
  | Remover membros          |        |   ✓   |            ✓           |
  | Alterar papel do usuário |        |   ✓   |            ✓           |
  | Painel do admin          |        |   ✓   |            ✓           |
  | Configurar SSO/segurança |        |   ✓   |            ✓           |
  | Gerenciar faturamento    |        |   ✓   |            ✓           |
  | Ver análises             |        |   ✓   |            ✓           |
  | Gerenciar acesso         |        |   ✓   |            ✓           |
  | Definir controles de uso |        |   ✓   |            ✓           |
  | Requer assento pago      |    ✓   |   ✓   |                        |
</div>
...
Todos os membros da equipe podem convidar outras pessoas. No momento, a gente não controla convites.

<div id="add-member">
  ### Adicionar membro
</div>

Adiciona membros de três formas:

1. **Convite por e-mail**

   * Clica em `Invite Members`
   * Digita os endereços de e-mail
   * As pessoas recebem convites por e-mail

2. **Link de convite**

   * Clica em `Invite Members`
   * Copia `Invite Link`
   * Compartilha com os membros da equipe

3. **SSO**
   * Configura o SSO no [admin dashboard](/pt-BR/account/teams/sso)
   * Usuários entram automaticamente ao fazer login com o e-mail do SSO

<Warning>
  Links de convite têm um prazo de expiração longo — qualquer pessoa com o link pode entrar.
  Revoga os links ou usa [SSO](/pt-BR/account/teams/sso)
</Warning>
...
Admins podem remover membros a qualquer momento pelo menu de contexto → "Remove". Se um membro tiver usado créditos, a vaga dele permanece ocupada até o fim do ciclo de cobrança.

<div id="change-role">
  ### Alterar função
</div>

Admins podem alterar as funções de outros membros clicando no menu de contexto e usando a opção "Change role".<br />

Deve haver pelo menos um Admin e um membro pago na equipe o tempo todo.

<div id="security-sso">
  ## Segurança e SSO
</div>

O SAML 2.0 Single Sign-On (SSO) está disponível nos planos Team. Principais recursos:

* Configurar conexões de SSO ([saiba mais](/pt-BR/account/teams/sso))
* Configurar a verificação de domínio
* Provisionamento automático de usuários
* Opções de exigência de SSO
* Integração com provedores de identidade (Okta, etc.)

<Note>
  <p className="!mb-0">A verificação de domínio é obrigatória para habilitar o SSO.</p>
</Note>
...
Acessa as configurações de uso para:

* Ativar cobrança por uso
* Ativar para modelos premium
* Definir alterações apenas para admins
* Definir limites mensais de gastos
* Monitorar o uso da equipe inteira
...
Ao adicionar membros ao time:

* Cada membro ou admin ocupa uma licença faturável (confere os [preços](https://cursor.com/pricing))
* Novos membros são cobrados proporcionalmente pelo tempo restante no período de cobrança
* Licenças de admin não pago não são contabilizadas

Adições no meio do mês cobram apenas pelos dias usados. Ao remover membros que usaram créditos, a licença deles permanece ocupada até o fim do ciclo de cobrança — não há reembolso proporcional.

Mudanças de função (por exemplo, de Admin para Admin não pago) ajustam a cobrança a partir da data da alteração. Escolhe cobrança mensal ou anual.

A renovação mensal/anual acontece na tua data original de cadastro, independentemente de mudanças no time.

<div id="switch-to-yearly-billing">
  ### Migrar para cobrança anual
</div>

Economiza **20%** ao migrar de mensal para anual:

1. Vai pro [Dashboard](https://cursor.com/dashboard)
2. Na seção de conta, clica em "Advanced" e depois em "Upgrade to yearly billing"

<Note>
  Só dá pra migrar de mensal pra anual pelo dashboard. Para migrar de
  anual pra mensal, fala com a gente em [hi@cursor.com](mailto:hi@cursor.com).
</Note>



# SCIM
Source: https://docs.cursor.com/pt-BR/account/teams/scim

Configure o provisionamento SCIM para gerenciamento automatizado de usuários e grupos

<div id="overview">
  ## Visão geral
</div>

O provisionamento SCIM 2.0 gerencia automaticamente os membros do teu time e os grupos do diretório pelo teu provedor de identidade. Disponível em planos Enterprise com SSO ativado.

<product_visual type="screenshot">
  Painel de configurações do SCIM mostrando a configuração do Active Directory Management
</product_visual>
...
* Plano Cursor Enterprise
* SSO deve ser configurado primeiro — **SCIM requer uma conexão SSO ativa**
* Acesso de admin ao teu provedor de identidade (Okta, Azure AD, etc.)
* Acesso de admin à tua organização no Cursor
...
Usuários são adicionados automaticamente ao Cursor quando atribuídos ao aplicativo SCIM no teu provedor de identidade. Quando removidos da atribuição, eles são excluídos. As alterações são sincronizadas em tempo real.

<div id="directory-groups">
  ### Grupos de diretório
</div>

Grupos de diretório e suas respectivas associações são sincronizados a partir do teu provedor de identidade. O gerenciamento de grupos e usuários deve ser feito pelo teu provedor de identidade — o Cursor exibe essas informações como somente leitura.

<div id="spend-management">
  ### Gestão de gastos
</div>

Define limites de gasto por usuário diferentes para cada grupo de diretório. Limites de grupos de diretório têm precedência sobre limites no nível da equipe. Usuários em vários grupos recebem o maior limite de gasto aplicável.
...
<Steps>
  <Step title="Verifica se o SSO está configurado">
    O SCIM exige que o SSO esteja configurado primeiro. Se ainda não configurou o SSO,
    segue o [guia de configuração de SSO](/pt-BR/account/teams/sso) antes de continuar.
  </Step>

  <Step title="Acessa o Active Directory Management">
    Acessa
    [cursor.com/dashboard?tab=active-directory](https://www.cursor.com/dashboard?tab=active-directory)
    com uma conta de admin, ou vai nas configurações do teu dashboard e seleciona a
    aba "Active Directory Management".
  </Step>

  <Step title="Inicia a configuração do SCIM">
    Quando o SSO estiver verificado, vai aparecer um link para a configuração passo a passo do SCIM. Clica
    nele pra iniciar o assistente de configuração.
  </Step>

  <Step title="Configura o SCIM no teu provedor de identidade">
    No teu provedor de identidade: Cria ou configura teu app SCIM. Usa
    o endpoint e o token de SCIM fornecidos pelo Cursor. Habilita o provisionamento de usuários e de grupos.
    Testa a conexão.
  </Step>

  <Step title="Configura limites de gasto (opcional)">
    De volta na página de Active Directory Management do Cursor: Vê teus grupos de diretório
    sincronizados. Define limites de gasto por usuário para grupos específicos, conforme necessário.
    Revisa quais limites se aplicam a usuários em vários grupos.
  </Step>
</Steps>
...
Pra instruções específicas por provedor:

<Card title="Guias do Provedor de Identidade" icon="book" href="https://workos.com/docs/integrations">
  Instruções de configuração para Okta, Azure AD, Google Workspace e outros.
</Card>
...
<Warning>
  Todo o gerenciamento de usuários e grupos deve ser feito pelo teu provedor de identidade.
  As alterações feitas no teu provedor de identidade serão sincronizadas automaticamente com o Cursor, mas
  não dá para modificar usuários ou grupos diretamente no Cursor.
</Warning>
...
* Adiciona usuários atribuindo eles ao teu aplicativo SCIM no teu provedor de identidade
* Remove usuários desatribuindo eles do aplicativo SCIM
* Alterações no perfil do usuário (nome, e-mail) sincronizam automaticamente a partir do teu provedor de identidade

<div id="group-management">
  ### Gerenciamento de grupos
</div>

* Grupos do diretório são sincronizados automaticamente a partir do teu provedor de identidade
* Alterações na associação a grupos são refletidas em tempo real
* Usa grupos para organizar usuários e definir limites de gasto diferentes

<div id="spend-limits">
  ### Limites de gasto
</div>

* Define limites diferentes por usuário para cada grupo do diretório
* Usuários herdam o maior limite de gasto entre seus grupos
* Limites de grupo substituem o limite padrão por usuário da equipe
...
<div id="why-isnt-scim-management-showing-up-in-my-dashboard">
  ### Por que o gerenciamento de SCIM não aparece no meu painel?
</div>

Garante que o SSO esteja configurado corretamente e funcionando antes de configurar o SCIM. O SCIM requer uma conexão de SSO ativa para funcionar.

<div id="why-arent-users-syncing">
  ### Por que os usuários não estão sincronizando?
</div>

Confere se os usuários estão atribuídos ao app do SCIM no teu provedor de identidade. Os usuários precisam ser atribuídos explicitamente para aparecer no Cursor.

<div id="why-arent-groups-appearing">
  ### Por que os grupos não estão aparecendo?
</div>

Verifica se o provisionamento de grupos por push está habilitado nas configurações de SCIM do teu provedor de identidade. A sincronização de grupos deve ser configurada separadamente da sincronização de usuários.

<div id="why-arent-spend-limits-applying">
  ### Por que os limites de gasto não estão sendo aplicados?
</div>

Confirma se os usuários estão corretamente atribuídos aos grupos esperados no teu provedor de identidade. A associação ao grupo determina quais limites de gasto se aplicam.

<div id="can-i-manage-scim-users-and-groups-directly-in-cursor">
  ### Posso gerenciar usuários e grupos do SCIM diretamente no Cursor?
</div>

Não. Todo o gerenciamento de usuários e grupos deve ser feito pelo teu provedor de identidade. O Cursor exibe essas informações como somente leitura.

<div id="how-quickly-do-changes-sync">
  ### Com que rapidez as alterações são sincronizadas?
</div>

As alterações feitas no teu provedor de identidade são sincronizadas com o Cursor em tempo real. Pode haver um breve atraso em operações em lote grandes.
